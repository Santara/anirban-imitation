...
def main():
    ...
    if args.eval_only:
        ...
        if False:
            print "AS: How did I get executed? :O \n"
            ...
            for i_traj in xrange(n):
                ...
                totalr = 0.
                l = 0
                while not sim.done and l < mdp.env_spec.timestep_limit:
                ...
                lengths.append(l)
        if args.ipython_after_eval:
            import IPython; IPython.embed()
        elif args.eval_log is not None:
            ...
            print "Mean return = {}, std return = {}, mean length = {}, std length = {}\n".format(np.asarray(returns).mean(), np.asarray(returns).std(), np.asarray(lengths).mean(), np.asarray(lengths).std())
